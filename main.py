    ...
    while attempts < max_attempts:
        ...
        if answer == "ДА":
            filtered_data = [
                ...
                if (converted_amount := get_converted_amount(transaction, use_mock=use_mock)) is not None
            ]
            # print(filtered_data)
            return filtered_data
        elif answer == "НЕТ":
    ...
    while attempts < max_attempts:
        ...
        if answer == "ДА":
            filtered_data = process_bank_search(
                filtered_data, search=input("Введите слово для поиска транзакций: ").lower()
            )
            # print(sorted_data)
            return filtered_data
        elif answer == "НЕТ":
...
def main(use_mock=False):
    ...
    result = ask_for_rub_convert(result, use_mock=use_mock)
    if result is None:
